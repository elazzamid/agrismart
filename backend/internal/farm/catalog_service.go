...
func NewCatalogService(db *pgxpool.Pool) *CatalogService { return &CatalogService{db: db} }
...
func (s *CatalogService) ListGrowthStages(ctx context.Context, cropID string) ([]CropGrowthStage, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&stage.ID, &stage.CropID, &stage.Name, &stage.SequenceNo, &stage.MinDays, &stage.MaxDays, &stage.Description); err != nil {
			return nil, err
		}

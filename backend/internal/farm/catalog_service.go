...
func (s *CatalogService) ListCrops(ctx context.Context) ([]Crop, error) {
	...
	for rows.Next() {
		var crop Crop
		if err := rows.Scan(&crop.ID, &crop.Code, &crop.Name, &crop.Description, &crop.IsActive); err != nil { return nil, err }
...
func (s *CatalogService) GetCrop(ctx context.Context, cropID string) (Crop, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) { return Crop{}, ErrNotFound }
	return crop, err
}
...
func (s *CatalogService) ListVarieties(ctx context.Context, cropID string) ([]CropVariety, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&variety.ID, &variety.CropID, &variety.Name, &variety.Description, &variety.IsActive); err != nil { return nil, err }
...
func (s *CatalogService) ListGrowthStages(ctx context.Context, cropID string) ([]CropGrowthStage, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&stage.ID, &stage.CropID, &stage.Name, &stage.SequenceNo, &stage.MinDays, &stage.MaxDays, &stage.Description); err != nil { return nil, err }

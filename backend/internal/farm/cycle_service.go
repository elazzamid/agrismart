...
type CropCycleService struct { db queryer }
func NewCropCycleService(db queryer) *CropCycleService { return &CropCycleService{db: db} }
...
func (s *CropCycleService) Create(ctx context.Context, farmerID, plotID string, in CreateCropCycleInput) (CropCycle, error) {
	...
	if strings.TrimSpace(in.PlantingDate) == "" { return CropCycle{}, errors.New("planting date is required") }
	var c CropCycle
	err := s.db.QueryRow(ctx, `
		...
		plotID, farmerID, in.CropID, in.VarietyID, in.PlantingDate,
	).Scan(&c.ID, &c.PlotID, &c.CropID, &c.VarietyID, &c.PlantingDate, &c.Status)
	if errors.Is(err, pgx.ErrNoRows) { return CropCycle{}, ErrCropCycleNotFound }
	return c, err
}
...
func (s *CropCycleService) List(ctx context.Context, farmerID, plotID string) ([]CropCycle, error) {
	...
	for rows.Next() {
		var c CropCycle
		if err := rows.Scan(&c.ID, &c.PlotID, &c.CropID, &c.VarietyID, &c.PlantingDate, &c.Status); err != nil { return nil, err }

...
func (s *Service) CreateDocument(ctx context.Context, slug, title, summary, author string) (Document, error) {
	...
	if slug == "" || title == "" { return Document{}, errors.New("slug and title are required") }
	var d Document
...
func (s *Service) Validate(ctx context.Context, documentID string, versionID, validator, decision, notes string) error {
	...
	if decision == "rejected" { status = "draft" }
	if _, err = tx.Exec(ctx, `UPDATE knowledge_documents SET status = $2, updated_at = NOW() WHERE id = $1`, documentID, status); err != nil { return err }

...
JOIN fertilizer_nutrients fn ON fn.fertilizer_id=f.id
WHERE fn.nutrient_code = ANY($1)
ORDER BY f.name, f.id, fn.nutrient_code`, normalized, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var code string
		var pct float64
		if err := rows.Scan(&item.ID, &item.Name, &item.Formulation, &code, &pct); err != nil {
			return nil, err
		}
		idx, ok := byID[item.ID]
		if !ok {

...
WITH limited_fertilizers AS (
	SELECT id, name, formulation, description
	FROM fertilizers
	ORDER BY name, id
	LIMIT $1
)
...
FROM limited_fertilizers f
LEFT JOIN fertilizer_nutrients fn ON fn.fertilizer_id=f.id
ORDER BY f.name, f.id, fn.nutrient_code`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.Name, &item.Formulation, &item.Description, &code, &percentage); err != nil {
			return nil, err
		}
		idx, ok := byID[item.ID]
		if !ok {
		...
		if code != "" {
			result[idx].Components = append(result[idx].Components, FertilizerComponent{NutrientCode: code, Percentage: percentage})
		}
	}
